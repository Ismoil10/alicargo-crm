...
Оплатите его, нажав кнопку "Мои заказы" ниже';
	...
	static function send_doc($lang){
		if ($lang=='uz') {
			$text = "<b>Jo'natma yuborishingiz mumkin</b>";
		}
		
		if ($lang=='ru') {
			$text = "<b>Вы можете отправить рассылку</b>";
		}
		return $text;
	}
	...
	static function confirm_doc($lang){
		if ($lang=='uz') {
			$text = "<b>Mijozlarga jo'natma yuborishni tasdiqlaysizmi?</b>";
		}
		
		if ($lang=='ru') {
			$text = "<b>Подтвердить отправку?</b>";
		}
		return $text;
	}

	static function finish_doc($lang){
		if ($lang=='uz') {
			$text = "<b>Jo'natma yuborildi</b>";
		}
		
		if ($lang=='ru') {
			$text = "<b>Рассылка отправлена</b>";
		}
		return $text;
	}

	static function cancel_doc($lang){
		if ($lang=='uz') {
			$text = "<b>Jo'natma yuborish bekor qilindi</b>";
		}
		
		if ($lang=='ru') {
			$text = "<b>Отправка отменена</b>";
		}
		return $text;
	}
