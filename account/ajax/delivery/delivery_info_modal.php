...
$location = db::arr_s("SELECT * FROM `ac_location` WHERE `ORDER_ID` = '$order_id' AND `TYPE` = 'dostavka'");
?>
...
<script>
    function getImage(){
    form = new FormData();
    form.append("ID", <?=$_POST['order_id']?>);
    js_ajax_post("viho/delivery/info_images.php",form).done(data=>{
      $(".imageBox").html(data);
    });
  }
  getImage();
