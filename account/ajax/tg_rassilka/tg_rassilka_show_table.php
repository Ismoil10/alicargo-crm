...
<?
$tg_users = db::arr_by_id("SELECT * FROM `tg_users` WHERE ID IN (SELECT USER_ID FROM `message_log` WHERE `RASSILKA_ID` = '$_POST[rassilka_id]' AND `MESSAGE_ID` > '0' )
");
?>
...
$data = db::arr("
SELECT * FROM `message_log` WHERE `RASSILKA_ID` = '$_POST[rassilka_id]' AND `MESSAGE_ID` > '0' 
LIMIT $start,10");
...
?>		
		...
		<?foreach($data as $v):?>	
			...
			<tr role="row" class = "odd">
				<td><?=$v['SEND_DATE']?></td>
				<td><?=$tg_users[$v['USER_ID']]['ISM_FAMILIYA']?></td>
				<td><?=$tg_users[$v['USER_ID']]['CODE']?></td>
